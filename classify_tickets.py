...
Пайплайн:
  1. Препроцессинг текста (нижний регистр → токенизация → лемматизация pymorphy3 → стоп-слова)
  2. TF-IDF
...
# ──────────────────────────────────────────────────────────────────────────────
# 4. Препроцессинг текста: нормализация + лемматизация + стоп-слова
# ──────────────────────────────────────────────────────────────────────────────
...
#
# ВАЖНО про порядок: сначала лемматизация, потом фильтр стоп-слов.
# Словарь NLTK хранит НОРМАЛЬНЫЕ формы («быть», «мочь», «этот»), поэтому
# проверка по сырому токену пропустит словоформы «была», «могу», «этого».
# Сначала приводим к лемме — потом сравниваем со списком.

print("\n" + "=" * 70)
print("Препроцессинг текста: лемматизация + стоп-слова")
print("=" * 70)

# NLTK-словарь покрывает только базовые служебные слова (~150 шт.). В тикетах
# поддержки дейтингового приложения через каждое сообщение идёт «мусор»,
# который NLTK не ловит, но который не различает категории:
#   - приветствия и прощания
#   - вежливость
#   - разговорные филлеры
#   - название продукта (встречается во ВСЕХ тикетах независимо от категории)
EXTRA_STOPWORDS = {
    # Приветствия / обращения
    "здравствуйте", "здравствуй", "привет", "приветствовать",
    "добрый", "доброе", "день", "вечер", "утро", "ночь",
    # Вежливость (леммы)
    "пожалуйста", "спасибо", "благодарить", "благодарность",
    "извинить", "извиняться", "простить",
    # Разговорные филлеры и общеупотребительные глаголы
    "просто", "очень", "вообще", "типа", "короче", "вроде",
    "мочь", "хотеть", "сделать", "делать",
    # Название продукта и общие слова про приложение
    "twinby", "твинби", "приложение", "апп", "программа",
    # Обращения к команде поддержки
    "ребята", "коллеги", "команда",
}

RU_STOPWORDS = set(stopwords.words("russian")) | EXTRA_STOPWORDS
print(f"Стоп-слова: NLTK {len(stopwords.words('russian'))} + "
      f"кастомных {len(EXTRA_STOPWORDS)} = {len(RU_STOPWORDS)}")
...
def preprocess_text(text: str) -> str:
    """Нижний регистр → токенизация → лемматизация → удаление стоп-слов.

    Порядок важен: фильтр стоп-слов применяется уже к ЛЕММАМ, потому что
    словарь стоп-слов хранится в нормальной форме. Иначе словоформы вроде
    «была», «могу», «этого» проскочат сквозь фильтр.
    """
    if not text:
        return ""
    text = text.lower()
